...
urgent_words = {
"urgent", "asap", "immediately", "now", "help", "cannot", "can't",
    "not working", "down", "error", "failed", "issue", "problem", "login", "critical", "pressing", "priority", "instant", "quickly", "unable", "blocked",
"broken", "offline", "outage", "fault", "crash", "anomaly", "glitch",
"bug", "credentials", "access", "auth", "assistance", "support"
}
...
def build_features(df: pd.DataFrame)->pd.DataFrame:
    df = df.copy()
    print("build_features RUNNING")
    msg = df["customer_message"].fillna("").astype(str)

    df["feat_char_len"] = msg.apply(text_length)  #longer messages = frustation
